...
- 标准表按维度分别出现 `ID + 答案`、导致两个同名 `ID` 表头时，不得删列、改表头或拿任一ID列代替全部维度。必须在 Manifest 的 `standard.dimension_bindings` 为每个维度声明自己的 `id_index + value_index`，并把绑定原样写入证据。
- 同一学员的维度位于不同源文档时，每个 `homework.documents` 项必须声明 `dimension` 或 `dimensions`。读取证据原样带回维度身份；评分时只按“姓名 + 维度 + 该维度ID”合并，布局模板和证据缓存也按维度隔离。禁止为了兼容旧评分器伪造合并表。
...
- 每个维度拥有独立ID集合。存在 `dimension_bindings` 时，只能使用该维度绑定的ID列和答案列；不同维度之间绝不借用ID、行号或布局缓存。
